...
<xsl:template match="/properties"><xsl:value-of select="$separator"/>
  <xsl:call-template name="create-comment">
    <xsl:with-param name="text" select="basic-details/."/>
...
<xsl:template match="group[count(property[customization/.='basic'])>0]">
...
# <xsl:value-of select="substring($gsep,1,7 + string-length(@name))"/>
#
<xsl:for-each select="property[customization/.='basic']">#   <xsl:value-of select="concat(name,$cr)"/></xsl:for-each>
<xsl:call-template name="create-comment">
...
<xsl:template match="property[customization/.='basic']">
