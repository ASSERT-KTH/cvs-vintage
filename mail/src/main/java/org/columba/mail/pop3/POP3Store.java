...
/**
 * First abstractionlayer of the POP3 Protocol. Its task is to manage the state
...
public class POP3Store implements Observer {
	 ...
	 */
	public int getSize(int index) throws IOException, POP3Exception,
	 ...
	 */
	public boolean deleteMessage(Object uid) throws CommandCancelledException,
	...
	public InputStream fetchMessage(int index) throws IOException,
	...
	protected void login() throws IOException, POP3Exception,
		...
		while (!login) {
			if ((password = Blowfish.decrypt(popItem.getRoot().getAttribute(
					"password", ""))).length == 0) {
				dialog = new PasswordDialog();

				dialog.showDialog(MessageFormat
						.format(MailResourceLoader.getString("dialog",
								"password", "enter_password"), new Object[] {
								popItem.get("user"), popItem.get("host") }),
						popItem.get("password"), popItem
	...
	public List checkSupportedAuthenticationMethods()
			throws CommandCancelledException, IOException {
		...
		try {
			...
			if (protocol.isApopSupported()) {
				supportedMechanisms
						.add(new Integer(AuthenticationManager.APOP));
			}
		} catch (POP3Exception e) {
			// APOP not supported
		}
	 ...
	 */
	private Map getUidMap() throws CommandCancelledException, IOException,
			POP3Exception {
		if (uidMap == null) {
			if (getMessageCount() != 0) {
				...
				for (int i = 0; i < uidList.length; i++) {
					uidMap.put(uidList[i].getUid(), new Integer(uidList[i]
							.getIndex()));
				}
			} else {
