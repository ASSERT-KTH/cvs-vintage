...
public class BounceCommand extends FolderCommand {
	...
	public BounceCommand(DefaultCommandReference reference) {
		super(reference);
	}
	...
	public void execute(WorkerStatusController worker) throws Exception {
		...
		SendableMessage sendableMessage = new MessageComposer(model)
				.compose(worker);
		...
		try {
			...
			FolderCommandReference ref = new FolderCommandReference(sentFolder);
			ref.setMessage(sendableMessage);
	...
	/**
	 * 
	 * Fill the <code>ComposerModel</code> with headerfields, bodytext and
	 * mimeparts.
	 * 
	 * This is a special method for the "Bounce Message" action.
	 * 
	 * @param message
	 *            The <code>Message</code> we want to edit as new message.
	 * 
	 * @param model
	...
	public static void bounceMessage(ColumbaMessage message, ComposerModel model) {
		...
		/*
		 * Hashtable hashtable = header.getHashtable(); for (Enumeration e =
		 * hashtable.keys(); e.hasMoreElements();) { Object key =
		 * e.nextElement();
		 * 
		 * try { model.setHeaderField( (String) key, (String)
		 * header.get((String) key)); } catch (ClassCastException ex) {
		...
		if (strToken.countTokens() == 2) {
			...
			// RFC0288 describes, that we can until we have found an >
			// character, so let use do this
			...
			if (strToken.countTokens() > 0) {
				...
				// there is an Bug and at this time we send this to the
				// commandline
				System.out
						.println("BUG found: ComposerWorker.Operation.COMPOSER_BOUNCE number 1");
			}
		} else {
			// there is an Bug and at this time we send this to the commandline
			System.out
					.println("BUG found: ComposerWorker.Operation.COMPOSER_BOUNCE number 2");
		}
