 ...
 *
 * --------------------------------------------------------------------------
 * $Id: ProcessesManager.java,v 1.4 2004/09/01 11:02:41 benoitf Exp $
 * --------------------------------------------------------------------------
 */
...
/**
 * Class <code>ProcessesManager</code> Provide a Process Manager for
...
public class ProcessesManager implements RemoteProcessesManager {
    ...
    /**
     * clean process environment this method just clean the hastable and
     * directory it not stop the process
    ...
    /**
     * recursive delting of a directories if existe this method only allow to
     * make deletion of File directory inside the current directory Your are not
     * allow to delete other directory and the directory is not deleted in this
     * case
    ...
    /**
     * create a directory inside the current directory if the directory existe
     * do nothing
     ...
     * @param String [] env property: "pkey=pvalue" can be null if there is no
     *        proprerty
     ...
     * @throws ProcessException if an exception occurs at bootstrapting
     */
    public String startJVM(JVMConfiguration jvmConf, String[] envp, String processDir) throws ProcessException,
            RemoteException {
     ...
     * @param String [] env property: "pkey=pvalue" can be null if there is no
     *        proprerty
     * @param String processDir directory where to launch the process (inside
     *        the current directory and without file separator);
     * @param String the process id
     * @throws RProcessException if an exception occurs at bootstrapting
     */
    public void startJVM(JVMConfiguration jvmConf, String[] envp, String processDir, String id)
            throws ProcessException, RemoteException {
     ...
     * @throws ProcessException if an exception occurs at bootstrapting
     */
    public String startProcess(String processLine, String[] envp, String processDir) throws ProcessException,
            RemoteException {
        String newid = getNewID();
        startProcess(processLine, envp, processDir, newid);
        return newid;
     ...
     * @throws RProcessException if an exception occurs at bootstrapting
     */
    public void startProcess(String processLine, String[] envp, String processDir, String id) throws ProcessException,
            RemoteException {
        Process p = null;
        if (verbose) {
        ...
        if ((commands.get(id) != null) || (processes.get(id) != null) || (directories.get(id) != null)) {
            ...
            try {
                ...
                String processOut = "\nProcess output  :\n" + getProcessError(p);
                cleanProcess(id);
                throw new ProcessException("The Process just stop after starting, exit value= " + ev + processErr
                        + processOut);
     ...
     * @return true if the Process is always alive and false if this Process
     ...
     * @throws ProcessException if - the id doen'st existe (with the
     ...
     * @throws ProcessException if: - The Process id doesn't exist - The Process
     *         process is stop
     */
     ...
     * @throws ProcessException if - the id doen'st existe
     */
    ...
    public void sendFile(String dirName, String fileName, byte[] b) throws RemoteException {
        try {
            ...
            if (dirC != null) {
                FileOutputStream fou = new FileOutputStream(new File(dirC + System.getProperty("file.separator")
                        + fileName));
                fou.write(b);
                fou.flush();
                fou.close();
