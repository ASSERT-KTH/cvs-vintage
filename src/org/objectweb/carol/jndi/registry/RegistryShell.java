...
public class RegistryShell {
	 ...
	 *     p [port number] rmi port number for this daemon (Optional, default 1099)
	 */
	public static void main(String args[]) {
		...
		try {
			System.out.println(
				"Connecting to Registry Manager: rmi://"
					+ HOST_LOCATION
					+ ":"
					+ PORT_NUMBER);
			System.out.println(
				"Start entering lines to manage remote processes");
	...
	public static void startFireWallLine(String line, boolean write) {
		try {
			if (line.trim().equals("")
				|| (line.equalsIgnoreCase("fw"))
				|| (line.equalsIgnoreCase("fr"))) {
				// nothing ...
			} else if (
				(line.trim().startsWith("help"))
					|| (line.trim().startsWith("?"))) {
	...
	private static void printIsAllowed(String line, boolean write) {
		try {
			...
			if (st.countTokens() < 2) {
				System.out.println(
					"the isa function take a process name parametter:");
				...
				if (write) {
					System.out.println(
						"Allow address for writing is: "
							+ getRegistryManager().isWriteAllow(
								Inet4Address.getByAddress(ip)));
				} else {
					System.out.println(
						"Allow address for reading is: "
							+ getRegistryManager().isReadAllow(
								Inet4Address.getByAddress(ip)));
				}
			}
		} catch (Exception e) {
	...
	private static void printAddAllow(String line, boolean write) {
		try {
			...
			if (st.countTokens() < 2) {
				System.out.println(
					"the aaa function take a process name parametter:");
				...
				byte[] ip = getIP(hostName);
				if (write) {
					getRegistryManager().addWriteAllowAddress(
						Inet4Address.getByAddress(ip));
				} else {
					getRegistryManager().addReadAllowAddress(
						Inet4Address.getByAddress(ip));
				}
			}
		} catch (Exception e) {
	...
	private static void printAddForbid(String line, boolean write) {
		try {
			...
			if (st.countTokens() < 2) {
				System.out.println(
					"the afa function take a process name parametter:");
				...
				byte[] ip = getIP(hostName);
				if (write) {
					getRegistryManager().addWriteForbidenAddress(
						Inet4Address.getByAddress(ip));
				} else {
					getRegistryManager().addReadForbidenAddress(
						Inet4Address.getByAddress(ip));
				}
			}
		} catch (Exception e) {
	...
	private static void printIsAll(String line, boolean write) {
		try {
			if (write) {
				System.out.println(
					"Allow address for writing is: "
						+ getRegistryManager().isWriteAllowAll());
			} else {
				System.out.println(
					"Allow address for reading is: "
						+ getRegistryManager().isReadAllowAll());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static void printListForbiden(String line, boolean write) {
		try {
			if (write) {
				...
				InetAddress[] fa =
					getRegistryManager().listWriteForbidenAddress();
				...
				InetAddress[] fa =
					getRegistryManager().listReadForbidenAddress();
				for (int i = 0; i < fa.length; i++) {
					System.out.println(fa[i]);
				}
			}
	...
	private static void printListAllowed(String line, boolean write) {
		try {
			if (write) {
				...
				InetAddress[] fa =
					getRegistryManager().listWriteAllowedAddress();
				for (int i = 0; i < fa.length; i++) {
					System.out.println(fa[i]);
				}
			} else {
				...
				InetAddress[] fa =
					getRegistryManager().listReadAllowedAddress();
				for (int i = 0; i < fa.length; i++) {
					System.out.println(fa[i]);
				}
			}
	...
	private static void printVerbose(String line) throws Exception {
		try {
			...
			if (st.countTokens() < 2) {
				System.out.println(
					"the verbose function take a process name parametter:");
				System.out.println("v <true|false>");
			} else {
				st.nextToken(); // verbose flag
				String verboseFlag = st.nextToken();
				getRegistryManager().setVerbose(
					new Boolean(verboseFlag).booleanValue());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static RegistryManager getRegistryManager() {
		...
		iprop.put(
			...
			"rmi://" + HOST_LOCATION + ":" + PORT_NUMBER);
		try {
			InitialContext in = new InitialContext(iprop);
			return (RegistryManager) in.lookup(
				ManageableRegistry.REGISTRY_MANAGER_NAME);
		} catch (Exception e) {
			System.out.println(
				"Can not start shell, can not contact Registry Manager" + e);
			return null;
		}
	}
	...
	public static void printHelpProcess() {
		...
		System.out.println(
			"purge						-> purge the registry");
		System.out.println(
			"list							-> list the registry");
	...
	public static void printFireWallHelpProcess() {
		...
		System.out.println(
			"lista						-> list allowed ip address");
		System.out.println(
			"listf						-> list forbiden ip address");
		System.out.println("aaa <IP>			    -> add allowed ip address");
		System.out.println(
			"afa	<IP>		    	-> add forbiden ip address");
		System.out.println(
			"aall			    			-> allowed all ip address");
		System.out.println(
			"fall <IP>			    	-> forbid all ip address");
		System.out.println(
			"is <IP>					-> is allow for this ip address");
		System.out.println(
			"isall			    		-> is allowed all ip address");
