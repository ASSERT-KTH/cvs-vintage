/*
 * @(#) MultiContext.java	1.0 27/01/2004
 *
...
/*
 * Class <code>MultiContext</code> is the CAROL JNDI SPI Context for multi Context management.
...
public class MultiContext implements Context {
	...
	if (TraceCarol.isDebugJndiCarol()) {
            TraceCarol.debugJndiCarol("MultiContext.listBindings(\""+name+"\")/rmi name=\""+cccf.getCurrentRMIName()+"\"");
        }
	try {
	...
	if (TraceCarol.isDebugJndiCarol()) {
            TraceCarol.debugJndiCarol("MultiContext.lookupLink(\""+name+"\")/rmi name=\""+cccf.getCurrentRMIName()+"\"");
        }
	try {
	...
	if (TraceCarol.isDebugJndiCarol()) {
            TraceCarol.debugJndiCarol("MultiContext.getNameParser(\""+name+"\")/rmi name=\""+cccf.getCurrentRMIName()+"\"");
        }
	try {
	...
	throws NamingException {
	if (TraceCarol.isDebugJndiCarol()) {
            TraceCarol.debugJndiCarol("MultiContext.composeName("+name+","+prefix+")/rmi name="+cccf.getCurrentRMIName());
        }
	return name;
    }
