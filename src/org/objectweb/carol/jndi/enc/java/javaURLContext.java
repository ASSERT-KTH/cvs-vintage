 ...
 *
 * --------------------------------------------------------------------------
 * $Id: javaURLContext.java,v 1.3 2004/09/01 11:02:41 benoitf Exp $
 * --------------------------------------------------------------------------
 */
...
/**
 * Context implementation for the "java:comp" namespace. Package must be named
 * .../java (See Initial Context) Most operations consist of retrieving the
 * actual CompNamingContext and sending it the operation for processing.
 * @author Philippe Durieux Contributor(s): Philippe Coq Monolog
 */
public class javaURLContext implements Context {
    ...
    /*
     * Resolve the name inside the javaURLContext Result must be a Context + the
     ...
     * @throws NameAlreadyBoundException if name is already bound
     * @throws javax.naming.directory.InvalidAttributesException if creation of
     *         the subcontext requires specification of mandatory attributes
     * @throws NamingException if a naming exception is encountered
    ...
    /**
     * Retrieves the full name of this context within its own namespace. Not
     * implemented.
     * @return this context's name in its own namespace; never null
     * @throws OperationNotSupportedException if the naming system does not have
    ...
    public Context getComponentContext() {
        ...
        //TODO: MHALAS: This is copied straight from Jonas NamingManager. I am
        // not sure
        ...
        if ((cl != null) && (cl.getParent() != null)) {
            ...
            if (ctx != null) {
                ...
                // MHALAS: This is new. If the context is not there, create new
                // context
                // This way the context for the component gets created
                // automaticall
                // first time somebody requests it
                try {
                    ctx = new CompNamingContext(cl.getParent().toString(), (Hashtable) myEnv.clone());
        ...
        if (ctx == null) {
            ...
            // first time somebody requests it
            try {
                ctx = new CompNamingContext(threadContext.toString(), (Hashtable) myEnv.clone());
            } catch (NamingException e) {
                TraceCarol.error("bad name");
            }
        } else {
            /*
             * try { TraceCarol.debugJndiCarol(
             * "name="+ctx.getNameInNamespace()); } catch (NamingException e) {
             * TraceCarol.error( "bad name"); }
             */
        }
    ...
    /**
     * gets the server component context This is used only internally in the
     * jonas NamingManager.
     */
