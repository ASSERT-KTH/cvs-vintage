...
public class POP3Store {
	...
	public POP3Store(PopItem popItem) {
		...
		try {
			handler = (POP3PreProcessingFilterPluginHandler) MainInterface.pluginManager
					.getHandler("org.columba.mail.pop3preprocessingfilter");
	...
	protected String modifyMessage(String rawString) {
		...
		//	</pop3preprocessingfilterlist>
		//
		XmlElement listElement = popItem
				.getElement("pop3preprocessingfilterlist");
		...
		for (int i = 0; i < listElement.count(); i++) {
			...
			try {
				...
				if (filterCache.containsKey(type) == true) {
					...
					filter = (AbstractPOP3PreProcessingFilter) handler
							.getPlugin(type, args);
				}
			} catch (Exception ex) {
				ex.printStackTrace();
