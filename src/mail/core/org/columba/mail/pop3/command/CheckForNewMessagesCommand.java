...
public class CheckForNewMessagesCommand extends Command {
	...
	public void execute(WorkerStatusController worker) throws Exception {
		FetchNewMessagesCommand command = new FetchNewMessagesCommand(
				getReferences());
		...
		try {
			...
			if ((newMessagesCount > 0)
					&& (server.getAccountItem().getPopItem()
	...
	protected void playSound() {
		...
		/*
		 * AccountItem item = server.getAccountItem(); PopItem popItem =
		 * item.getPopItem(); String file = popItem.get("sound_file");
		 * ColumbaLogger.log.fine("playing sound file=" + file); if
		 * (file.equalsIgnoreCase("default")) { PlaySound.play("newmail.wav"); }
		 * else { try { PlaySound.play(new URL("file:+" + file)); } catch
