package org.columba.mail.plugin;

import java.io.File;

import org.columba.core.xml.XmlElement;

/**
 * @author freddy
 *
 * To change this generated comment edit the template variable "typecomment":
 * Window>Preferences>Java>Templates.
 * To enable and disable the creation of type comments go to
 * Window>Preferences>Java>Code Generation.
 */
public class RemoteFilterPluginHandler extends AbstractFilterPluginHandler {

	/**
	 * Constructor for RemoteFilterPluginHandler.
	 */
	public RemoteFilterPluginHandler() {
		super(
			"filter_remote",
			"org/columba/mail/filter/filter_remote.xml",
			"filterlist");

	}

	public void addPlugin(String name, File pluginFolder, XmlElement element) {
		super.addPlugin(name, pluginFolder, element);

		XmlElement child = element.getElement("arguments/filter");

		parentNode.addElement(child);

	}
}
