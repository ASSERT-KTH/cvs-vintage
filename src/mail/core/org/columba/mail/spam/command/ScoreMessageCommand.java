...
public class ScoreMessageCommand extends FolderCommand {
     ...
     * @param references
     */
    public ScoreMessageCommand(FrameMediator frame, DefaultCommandReference[] refs) {
        super(frame, refs);
    }
     ...
     * @throws IOException
     */
    private boolean scoreMessage(int j, ProbabilityMap map)
    throws Exception, IOException {
        ...
        boolean result = SpamController.getInstance().scoreMessage(istream, map);
        ...
        if (item.getSpamItem().checkAddressbook()) {
            ...
            boolean isInAddressbook = 
                new AddressbookFilter().process(srcFolder, uids[j]);
