...
public class MimeParser {
	...
	public MimeParser(Rfc822Header h) {
		this.rfc822Header = h;
	...
	public MimePart parse(String input) {
		...
		message = MessageParserUtils.divideMessage(input);

		if (message != null) {
			mimeHeader = parseHeader(message[0]);

			if (mimeHeader.contentType.equals("multipart")) {

				MimeMultipartParser typeParser = new MimeMultipartParser();
				attachments = typeParser.parse(mimeHeader, message[1]);
			} else {

				MimeStandardParser typeParser = new MimeStandardParser();
				attachments = typeParser.parse(mimeHeader, message[1]);
			}
		...
		while (actLine != null) {
			...
			if (actLine.startsWith("Content-Type")) {
				...
				if (charPos != -1) {
					...
					if (charPos != -1) {
						output.contentType =
							actParam.substring(0, charPos).trim().toLowerCase();
						output.contentSubtype =
							actParam
								.substring(charPos + 1)
								.trim()
								.toLowerCase();
						...
						if (charPos != -1) {
							output.putContentParameter(
								actParam
									...
									.toLowerCase(),
								decoder.decode(
									actParam.substring(charPos + 1).trim()));
						}
					}
				...
				charPos = actLine.indexOf(':');
				if (charPos != -1) {
					output.contentTransferEncoding =
						actLine.substring(charPos + 1).trim().toLowerCase();
				}
				...
				if (charPos != -1) {
					output.contentDescription =
						actLine.substring(charPos + 1).trim();
				}
