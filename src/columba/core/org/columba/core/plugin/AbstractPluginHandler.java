...
public abstract class AbstractPluginHandler implements PluginHandler {
		...
		JOptionPane
				.showMessageDialog(
						...
						"Error while trying to instantiate plugin "
								...
								+ "This plugin will be disabled. After fixing the problem you have to manually enable it in the Plugin Manager.");
	...
	protected String getPluginClassName(String name, String id) {
		...
		for (int i = 0; i < count; i++) {
			 ...
			 * plugin // -> extract the correct id // example original id:
			 ...
			 * extract the correct id s = s.substring(s.indexOf('$')+1);
			 * System.out.println("class-id="+s); }
			 */
			if (name.equals(s)) {
