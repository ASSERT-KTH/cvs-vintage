...
/**
 * Provides Columba start-up without starting a new application.
 * (For use as part of an existing application)
 */
public class CustomStart {
    
    /**
     * Start Columba as a 'plugin' using default setup.
     */
    public static void startColumbaAsPlugin() {
        start(null, null);
    }
    
    /**
     * Start Columba as a 'plugin' - pass in 'command-line' arguments.
     */
    public static void startColumbaAsPlugin(String[] args) {
        ...
        start(new File(path), args);
        
        ColumbaServer.getColumbaServer().handleCommandLineParameters(args);
    }
    
    private static void start(File configPath, String[] args) {
    
        MainInterface.config = new Config(configPath);
