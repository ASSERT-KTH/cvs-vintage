...
public class XmlIOTest extends TestCase {
    ...
    public void testXmlElement() throws IOException {
        ...
        xmlIO = new XmlIO();
        assertTrue("Could not parse the written XML", xmlIO.load(new ByteArrayInputStream(baos.toByteArray())));
        XmlElement actual = xmlIO.getRoot().getElement(0);
        assertEquals("The original and the written XML element are not equal", expected, actual);
    }
    ...
    public void testReadInputStream() {
        ...
        assertTrue("The XML could not be loaded", xmlIO.load(new ByteArrayInputStream(expected.getBytes())));

        XmlElement actualXml = xmlIO.getRoot().getElement("xml");
        assertEquals("Name isnt correct", "xml", actualXml.getName());
        assertEquals("The first attribute isnt correct", "one", actualXml.getAttribute("attr"));
        assertEquals("The second attribute isnt correct", "two", actualXml.getAttribute("secAttr"));

        XmlElement child = actualXml.getElement(0);
        assertEquals("The child name isnt correct", "child", child.getName());
        assertEquals("The childs first attribute isnt correct", "other", child.getAttribute("name"));
    }
}
