...
public class ScarabAttribute extends BaseObject
{
    ...
    void setScarabAttributeType(ScarabAttributeType v) throws Exception
    {
...
//    private Vector tempcollScarabActivitys;
     ...
     * the collection. Otherwise if this ScarabAttribute is new, it will return
...
//    private Vector tempcollScarabAttributeOptions;
    ...
    {
        return getScarabAttributeOptions(new Criteria(5));
    }
...
*/
        ...
        else
        {
            ...
            boolean newCriteria = true;
                   criteria.add(ScarabAttributeOptionPeer.ATTRIBUTE_ID, getAttributeId() );               
               if ( lastScarabAttributeOptionsCriteria.size() == criteria.size() )
            {
                newCriteria = false;
                for (Enumeration e=criteria.keys(); e.hasMoreElements(); )
                {
                    Object key = e.nextElement();
                    if ( lastScarabAttributeOptionsCriteria.containsKey(key) )
                    {
                        if ( !criteria.get(key).equals(
                            lastScarabAttributeOptionsCriteria.get(key)) )
                        {
                            newCriteria = true;
                            break;
                        }
