...
//    private Vector tempcollScarabAttributeTypes;
     ...
     * the collection. Otherwise if this ScarabAttributeClass is new, it will return
    ...
    {
        return getScarabAttributeTypes(new Criteria(5));
    }
...
*/
        ...
        else
        {
            ...
            boolean newCriteria = true;
                   criteria.add(ScarabAttributeTypePeer.ATTRIBUTE_CLASS_ID, getAttributeClassId() );               
               if ( lastScarabAttributeTypesCriteria.size() == criteria.size() )
            {
                newCriteria = false;
                for (Enumeration e=criteria.keys(); e.hasMoreElements(); )
                {
                    Object key = e.nextElement();
                    if ( lastScarabAttributeTypesCriteria.containsKey(key) )
                    {
                        if ( !criteria.get(key).equals(
                            lastScarabAttributeTypesCriteria.get(key)) )
                        {
                            newCriteria = true;
                            break;
                        }
