 ...
 * to call ejbStore() on when a ejbFind() method is called within a transaction. EJB 2.0- 9.6.4
...
public class TxEntityMap
{
     ...
     */
    public synchronized void associate(Transaction tx, EntityEnterpriseContext entity)
    {
    ...
    /**
     * disassociate entity with transaction.  When the transaction has no more entities.
     * it is removed from this class's internal HashMap.
     */
    public synchronized void disassociate(Transaction tx, EntityEnterpriseContext ctx)
    {
	HashMap entityMap = (HashMap)m_map.get(tx);
	if (entityMap == null)
	{
