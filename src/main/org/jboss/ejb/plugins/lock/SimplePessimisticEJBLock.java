...
*  <ol>
*   <li>Did what was said above, moved to an extension based mech with an abstract base class.
*   <li>This is the simple lock, won't scale well (imho) but is robust in normal operation
*   <li>The class must now implement schedule and the various notification calls.  EndTransaction 
      ...
      try
      {
         ...
         if (miTx != null && miTx.getStatus() == Status.STATUS_MARKED_ROLLBACK)
         {
            log.error("Saw rolled back tx="+miTx);
				...
				//Wake up the next ones, this won't scale, under stress we will wake everyone... 
