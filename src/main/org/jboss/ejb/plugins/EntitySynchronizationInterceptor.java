...
*   It registers a callback with the underlying transaction monitor through the JTA interfaces.
*   If there is no transaction the policy is to store state upon returning from invocation.
...
*   <li>afterCompletion doesn't return to pool anymore, idea is to simplify design by not
*      mucking with reuse of the instances
*   <li>before completion checks for a rolledback tx and doesn't call the store in case of a rollback
