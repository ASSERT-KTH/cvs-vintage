...
/**
 * This class is used by the J2eeDeployer to create, remove or find a
 * particular Deployment. It uses the Installer class to create a Deployment.
 *      
...
public class InstallerFactory
{
    ...
    *  object as its representation.
    *  
    *  @param _src J2ee module (ejb/war/ear) to deploy
    *  @return a Deployment object representing the deployment
    *  
    *  @throws J2eeDeploymentException  if the module is not installable for 
    *                                   some reasons  (syntactical errors, ???)
    *  @throws IOException if a file operation (_src download jar file
    *                      extraction) fails
    */
   public Deployment install(URL _src)
   ...
   public Deployment[] getDeployments()
   {
      ...
      for (int i = 0, l = files.length; i<l; ++i)
      {
         ...
         if (deployment.exists())
         {
            ...
            {
               log.error("exception while searching deployments: " +
                         _ioe.getMessage());
            }
         }
      }
    ...
    *  
    *  @param _pattern  wether the name of the application or the src URL 
    *                   of the application (the one that was given on install
    *                   (URL))
   ...
   public Deployment findDeployment(String _pattern)
   {
      ...
      for (int i = 0, l = files.length; i<l; ++i)
      {
         if (realPattern.equals(files[i].getName()))
         {
            ...
            {
               log.error("exception while searching deployment: " +
                         _ioe.getMessage());
            }
         }
      }
   ...
   /**
    *  Does some cleanup in the deployments. Intended to remove files that
    *  didnt become removed in previous sessions because of the Win2k
    *  removal problems.
    *  
   ...
   public void unclutter() throws IOException
   {
      ...
      for (int i = 0, l = files.length; i<l; ++i)
      {
         ...
         if (dep.exists())
         {
            ...
            {
               log.error("exception while uncluttering deployment " +
                         files[i]+": "+_ioe.getMessage());
            }
         }
         else
         {
