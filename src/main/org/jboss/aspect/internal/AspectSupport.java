...
final public class AspectSupport implements AspectDefinitionConstants
{
   ...
   final static public Map loadAspects(URL configSource) throws AspectInitizationException
   {
      try
      {
         ...
         return loadAspects(doc);

      }
      catch (DocumentException e)
      {
         throw new AspectInitizationException(
            "Invalid Aspect configuration file: " + configSource,
            e);
      }
    ...
    * AspectDefinition objects.
    */
   final static public Map loadAspects(Document doc) throws AspectInitizationException
   {
   ...
   final static public ArrayList loadAspectInterceptorHolderList(
      Element xml,
      Map namedInterceptors,
      Map namedStacks)
      throws AspectInitizationException
   {
      ...
      while (iter.hasNext())
      {
         ...
         if (iXML.getQName().equals(ELEM_INTERCEPTOR_REF))
         {
            ...
            if (ref == null)
               throw new AspectInitizationException(
                  "No interceptor with name '" + name + "' has been defined.");
            v.add(ref);
         }
         ...
         if (iXML.getQName().equals(ELEM_STACK_REF))
         {
            ...
            if (stack == null)
               throw new AspectInitizationException(
                  "No stack with name '" + name + "' has been defined.");
   ...
   final static AspectInterceptorHolder loadInterceptorElement(Element xml, Map namedInterceptors)
      throws AspectInitizationException
   {
      String name = null;
      ...
      if (name != null)
      {
         ...
         if (old != null)
            throw new AspectInitizationException(
               "An interceptor with name '" + name + "' has allready been defined.");
      }
      return aih;
   }

   final static ArrayList loadStackElement(Element xml, Map namedInterceptors, Map namedStacks)
      throws AspectInitizationException
   {
      String name = null;
      if (xml.attribute(ATTR_NAME) != null)
      ...
      if (name != null)
      {
         ...
         if (old != null)
            throw new AspectInitizationException(
               "An stack with name '" + name + "' has allready been defined.");
      }
      return stack;
   }
   ...
   final static public Set getExposedMethods(Class[] interfaces)
   {
      ...
      for (int i = 0; i < interfaces.length; i++)
      {
         ...
         for (int j = 0; j < t.length; j++)
         {
            ...
            set.add(getMethodSignature(method));
         }
      }
      return set;
   }
   ...
   final static public String getMethodSignature(Method method)
   {
      StringBuffer rc = new StringBuffer();
      rc.append(method.getReturnType().getName());
      rc.append(" ");
      rc.append(method.getName());
      Class args[] = method.getParameterTypes();
      for (int i = 0; args != null && i < args.length; i++)
      {
         if (i != 0)
            rc.append(",");
         rc.append(args[i].getName());
      }
      return rc.toString();
   }
