...
#
# $Id: create-db.sh,v 1.26 2004/03/10 17:20:35 pledbrook Exp $
#
...
POPULATION_SCRIPT_DIR="${PATHNAME}/../../target/scarab/WEB-INF/sql"
...
if [ -d "${POPULATION_SCRIPT_DIR}/mysql" ] ; then
    dbtype="mysql"
elif [ -d "${POPULATION_SCRIPT_DIR}/postgresql" ] ; then
    dbtype="postgresql"
elif [ -d "${POPULATION_SCRIPT_DIR}/oracle" ] ; then 
    dbtype="oracle"
fi
...
for i in ${FILES} ; do
    SQL_FILE=${POPULATION_SCRIPT_DIR}/${dbtype}/${i}
    if [ ! -f ${POPULATION_SCRIPT_DIR}/${dbtype}/${i} ]; then
        SQL_FILE=${POPULATION_SCRIPT_DIR}/${i}
    fi
    
    if [ -z "${quiet}" ] ; then
        echo "Importing ${i}..."
    fi
    ${MYSQL} ${MYSQLCMD} ${DB_NAME} < ${SQL_FILE}
done
...
for i in ${FILES} ; do
    ...
    if [ -z "${quiet}" ] ; then
        echo "Importing ${i}..."
        ${PSQL} -f ${SQL_FILE} ${PSQLCMD} ${DB_NAME}
    else
        ${PSQL} -f ${SQL_FILE} ${PSQLCMD} ${DB_NAME} 2> /dev/null
    fi
done
...
for i in ${FILES} ; do 
    ...
    cat /tmp/${PPID}preamble.sql ${SQL_FILE} > /tmp/${PPID}pp.sql 
